"""下载 Robotiq 夹爪 URDF 与 mesh 文件。

从 ros-industrial-attic/robotiq 仓库下载 2F 140 / 2F 85 夹爪的 URDF 与 mesh
到本地。

用法：
    python scripts/download_robotiq.py
    python scripts/download_robotiq.py --output-dir /path/to/output
"""
...
OWNER = "ros-industrial-attic"
REPO = "robotiq"
BRANCH = "kinetic-devel"

# 两个夹爪型号的 visualization 包含 URDF + mesh
TREE_PATHS = [
    "robotiq_2f_140_gripper_visualization",
    "robotiq_2f_85_gripper_visualization",
]

DEFAULT_OUTPUT = Path("data/sources/web/robotiq/grippers")
TIMEOUT = 30.0


async def run(output_dir: Path) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    print("=== 下载 Robotiq 夹爪 URDF/mesh ===")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="下载 Robotiq 夹爪 URDF + mesh 文件"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
