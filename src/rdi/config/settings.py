...
class Settings(BaseSettings):
    ...
    # ─── 千问模型配置 ───
    qwen_api_key: str = Field(default="", description="阿里云百炼平台 API Key")
    qwen_model: str = Field(default="qwen-plus", description="千问模型名称")
    qwen_embedding_model: str = Field(default="text-embedding-v3", description="Embedding 模型")
    qwen_max_retries: int = Field(default=3, description="LLM 调用最大重试次数")
    qwen_temperature: float = Field(default=0.3, description="LLM 生成温度，目标解析用低温度")
    ...
    adapter_rate_limit: int = Field(default=10, description="每秒最大并发请求数")
    adapter_cache_ttl: int = Field(default=3600, description="缓存TTL秒数")

    # ─── ChromaDB 配置 ───
    chromadb_path: str = Field(default="./data/experience_db", description="ChromaDB持久化路径")

    # ─── 输出配置 ───
    output_dir: str = Field(default="./data/output_packages", description="数据包输出目录")
