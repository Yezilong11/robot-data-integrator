...
Robot Data Integrator (RDI) 是一个基于 LLM 驱动的智能工作流系统，用于从多种异构数据源自动检索、解析、校验和整合机器人操作与抓取研究所需的数据。系统集成 LangGraph 工作流编排、Qwen 大语言模型智能决策、Hermes 持续学习引擎，以及 15 种数据源适配器，最终输出标准化的实验数据包。前端采用 FastAPI + 原生 HTML/CSS/JS 实现工作区式界面，与设计稿（`docs/ui-redesign-mockup.html`）逐像素对齐。
...
- **FastAPI + 工作区式前端** — 原生 HTML/CSS/JS 实现 VSCode 风格三栏工作区，支持演示/真实两种运行模式、PDF 论文上传、数据包可视化审查

## 前端界面

工作区式三栏布局（GitHub 深色风 · 黑底橙调），与设计稿 `docs/ui-redesign-mockup.html` 对齐：

```
┌ 标题栏（待输入 / 运行中徽章 · LLM 引擎）
├ 活动栏 ┬ 左栏（资源管理器） ┬ 中栏（主视图） ┬ 右栏（检查器）
│  6 面板│  数据包文件树       │  灯带工作流      │  4 Tab 切换
│  + 新会话│  （可折叠/懒加载） │  + LLM 决策分析 │
│        │                    │  + 目标输出      │
├ 底部目标输入条（＋PDF 上传 · 目标输入 · ▶ 运行/继续运行）
└ 状态栏（待输入/运行中 · 当前阶段 · LLM 调用 · 耗时 · 错误）
```

- **活动栏** — 6 个面板切换（资源管理器 / 数据检索 / 决策 / 审查 / 日志 / 设置）+ 底部「新会话」按钮（清空输入并重置工作区）
- **左栏** — 数据包文件树：嵌套目录可折叠、数据包子目录懒加载、顶部刷新/全部展开/全部折叠
- **中栏** — 灯带工作流（检索→转换→校验→打包→审查 5 节点随运行实时点亮）+ LLM 决策分析（诚实标注「LLM 生成」/「规则兜底」）+ 目标输出（单行汇总，点击展开产物清单）
- **右栏** — 检查器 4 Tab（当前文件 / 需求状态 / 语义约定 / 质量校验）
- **交互** — 三栏均可拖拽调宽；运行期间工作区保持可见、灯带逐节点推进并显示当前阶段文字；LLM 调用允许最长 30 分钟等待；PDF 支持 10MB（覆盖十几页论文）
- **运行模式** — 演示流程（动画演示全流程）与真实流程（真实 LLM 调用），人机审查中断后可「继续运行」
...
├── frontend/         # 前端服务端渲染与运行逻辑库
│   └── app.py        #   工作区 HTML 渲染（build_workspace_html）+ run_graph / resume 工作流封装
└── static/           # 静态前端
    └── index.html    #   页面外壳 + 交互 JS（轮询/事件委托/拖拽/新会话）
```
...
# 运行端到端 demo（生成 URDF + STL + MJCF XML + grasp JSON 数据包）
